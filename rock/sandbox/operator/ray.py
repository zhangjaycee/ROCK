...
class RayOperator(AbstractOperator):
    ...
    async def restart(self, config: DockerDeploymentConfig, host_ip: str | None = None) -> SandboxInfo:
        ...
        Flow:
          1. Kill the old actor if it still exists. We don't call actor.stop()
             because that would invoke deployment._stop() and remove the container.
          2. Create a fresh detached actor pinned to ``host_ip`` (the node that
          ...
          3. actor.restart() → deployment.restart() — DockerDeployment.restart()
        ...
        async with self._ray_service.get_ray_rwlock().read_lock():
            ...
            try:
                ...
                logger.info(f"Actor {actor_name} already gone, proceeding with restart")
