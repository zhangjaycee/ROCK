...
class SandboxStateMachine(StateChart):
    ...
    async def on_restart(self, sandbox_id: str, operator, meta_store) -> None:
        ...
        # The previous stop() called meta_store.archive_cache() which removed the Redis
    ...
    async def on_delete(
        ...
        # Rebuild DockerDeploymentConfig from the spec snapshot persisted at
        # start time, same as on_restart. Without spec we cannot pin the new
        # delete-actor to the host that owns the container, so the docker rm
        # step is skipped and ContainerCleanupTask becomes the fallback.
        ...
        if spec:
            try:
                ...
                # operator.delete must not block the metadata soft-delete; the
                # ContainerCleanupTask cron sweep will reap the docker container later.
