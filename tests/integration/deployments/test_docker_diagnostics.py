...
- ``_get_container_status()``        — exercised as observable state during start/stop transitions
...
class TestDockerStartHelpers:
    ...
    def test_docker_start_revives_exited_container_with_same_id_and_ports(
        ...
        try:
            ...
            for _ in range(20):
                if deployment._get_container_status() == "exited":
                    break
                time.sleep(0.2)
            assert deployment._get_container_status() == "exited"

            deployment._docker_start()  # the function actually used by restart()

            # Container should be running again
            for _ in range(20):
                if deployment._get_container_status() == "running":
                    break
                time.sleep(0.2)
            assert deployment._get_container_status() == "running"
